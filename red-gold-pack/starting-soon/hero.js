...
(() => {
  ...
  /* ---- palette ---- */
  const NEON = '#d64a33', MINT = '#e8b349', BLADE = '#ffe9c0', PWR = '#ffce85';
  ...
  const FW_COLORS = ['#ff3df0', '#d64a33', '#e8b349', '#ffd84a', '#ffffff', '#ff7a52'];
  ...
  function drawPortal(cx, cy, w, h, T) {
    ...
    ctx.globalAlpha = 0.7 + 0.3 * Math.sin(T * 15); ctx.shadowColor = '#ffffff'; ctx.shadowBlur = 22; ctx.fillStyle = '#f5ece0';
  ...
  function drawAura(cx, cy, T, intensity, alpha) {
    ...
    // rising embers
    ctx.shadowBlur = 10; ctx.fillStyle = '#f5ece0';
    for (let i = 0; i < 7; i++) {
      const t = (T * 0.9 + i / 7) % 1;
  ...
  function draw(T) {
    ...
    if (s.pelletVis) {
      ...
      ctx.save(); ctx.shadowColor = MINT; ctx.shadowBlur = 22; ctx.fillStyle = '#f5ece0';
    ...
    if (s.sliceT >= 0) {
      ...
      if (ct > 0.40 && ct < 0.66) {                                   // radial impact flash, peaks at the slash
        ...
        g.addColorStop(0, 'rgba(255,255,255,0.95)'); g.addColorStop(0.35, 'rgba(255,200,120,0.5)'); g.addColorStop(1, 'rgba(214,74,51,0)');
