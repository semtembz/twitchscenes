/* ============================================================
   VELVET LOUNGE — alert engine ("The Gilt Room")
   ...
   Delta-clamped rAF GOLD-DUST burst (transparent canvas).
...
(() => {
  ...
  /* ---- per-event choreography table ----
     ...
     kicker : deco-caps event line
     sub    : little italic sub-line
     motes  : how many gold-dust motes the burst spits (raid/host bigger)
  ...
  /* ---- gold-dust burst on a transparent canvas (delta-clamped rAF) ---- */
  const cv = $(".fx");
  ...
  function spawnBurst(n) {
    ...
    for (let i = 0; i < n; i++) {
      burst.push({
        ...
        vx: R(20, 200) / 60, vy: R(-200, -50) / 60,
        g: R(90, 170) / 3600,             // gravity per frame-ish
  ...
  function drawMote(p) {
    // a tiny rotated gold diamond — a fleck of gold dust
    ctx.save();
  ...
  window.__velvetAlertDraw = frame; // rAF-independent verification hook
  ...
  function play(opts) {
    ...
    const name = (opts.name != null && opts.name !== "") ? String(opts.name) : "NewGuest";
    ...
    // gold-dust burst (staggered a touch so it reads after the slide-in)
    setTimeout(() => spawnBurst(cfg.motes), 220);
    // raid/host get a second pop for "bigger" feel
    if (cfg.motes >= 30) setTimeout(() => spawnBurst(Math.round(cfg.motes * 0.6)), 620);
  ...
  function renderPlay(opts) {
    ...
    const name = (opts.name != null && opts.name !== "") ? String(opts.name) : (params.get("name") || "NewGuest");
  ...
  function renderAdvance() {
    ...
    if (_cfg.motes >= 30 && ms >= 620 && !_b2) { _b2 = true; spawnBurst(Math.round(_cfg.motes * 0.6)); }
    if (ctx) frame();
    _f++;
    return ms;
  }
