...
(() => {
  ...
  /* ---- palette ---- */
  const NEON = '#ff5e9c', MINT = '#ffd1e6', BLADE = '#ffe9c0', PWR = '#ffce85';
  const MAG = '#ff3df0', MAG_SCARED = '#bfe9ff';
  const SKIN = '#e6bd9a', HAIR = '#19110b', CAPE = '#3a1220', OUTFIT = '#46221a', DOT = '#f4dca6';
  ...
  const FW_COLORS = ['#ff3df0', '#ff5e9c', '#ffd1e6', '#ffd84a', '#ffffff', '#ff7a52'];
  ...
  function drawSword(hx, hy, faceL, ang, powered, T) {
    ...
    // grip
    ctx.fillStyle = '#241410'; ctx.fillRect(-2, 0, 4, 9);
    // guard
  ...
  function drawPortal(cx, cy, w, h, T) {
    ...
    // dark void
    trace(); ctx.fillStyle = 'rgba(10,8,28,0.95)'; ctx.fill();
    ...
    ctx.save(); ctx.translate(-4, 0); ctx.strokeStyle = '#ffcf6a'; trace(); ctx.stroke(); ctx.restore();
    ctx.save(); ctx.translate(4, 0); ctx.strokeStyle = '#ff8a5c'; trace(); ctx.stroke(); ctx.restore();
    // bright ragged rim
    ...
    ctx.globalAlpha = 0.7 + 0.3 * Math.sin(T * 15); ctx.shadowColor = '#ffffff'; ctx.shadowBlur = 22; ctx.fillStyle = '#f7f2f5';
  ...
  function drawAura(cx, cy, T, intensity, alpha) {
    ...
    // rising embers
    ctx.shadowBlur = 10; ctx.fillStyle = '#f7f2f5';
    for (let i = 0; i < 7; i++) {
      const t = (T * 0.9 + i / 7) % 1;
  ...
  function draw(T) {
    ...
    if (s.pelletVis) {
      ...
      ctx.save(); ctx.shadowColor = MINT; ctx.shadowBlur = 22; ctx.fillStyle = '#f7f2f5';
    ...
    if (s.sliceT >= 0) {
      ...
      if (ct > 0.40 && ct < 0.66) {                                   // radial impact flash, peaks at the slash
        ...
        g.addColorStop(0, 'rgba(255,255,255,0.95)'); g.addColorStop(0.35, 'rgba(255,200,120,0.5)'); g.addColorStop(1, 'rgba(255,94,156,0)');
      ...
      if (ct > 0.46 && ct < 0.98) {                                   // sparks flung off the cut
        ...
        ctx.save(); ctx.fillStyle = '#ffe9c0'; ctx.shadowColor = MINT; ctx.shadowBlur = 12; ctx.globalAlpha = Math.max(0, 1 - life);
